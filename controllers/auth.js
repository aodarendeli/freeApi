...
const register = async (req, res) => {
    try {
        ...
        if (user) {
            return res.status(500).json({ message: "Bu email hesabı zaten var!" });
        }

        if (password.length < 6) {
            return res.status(500).json({ message: "Parola 6 haneden buyuk olmalı!" });
        }
...
const login = async (req, res) => {
    try {
        ...
        if (comparePassword) {
            return res.status(500).json({ message: "Paralo yanlış !" });
        }
